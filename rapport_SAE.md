...
Encadrement : Bilal Faye et Hanane Azzag
...
# 4. Strategie 1 — Trois modeles specialises
...
# 5. Strategie 2 — Modele multi-tache Mixture of Experts

## Architecture MoE

Au lieu de simples couches Dense en sortie, nous utilisons une architecture Mixture of Experts. Le backbone EfficientNetB0 est partage entre les trois taches. Apres le GlobalAveragePooling, BatchNormalization et Dropout, trois tetes MoE independantes predisent respectivement l'age, le genre et l'ethnicite.
...
# 6. Strategie 3 — Comparaison des backbones
...
Le modele n'utilise ni infrarouge ni capteur thermique. Il analyse uniquement les patterns statistiques dans les pixels de l'image. Ses predictions sont correlatives et non causales.


# 8. Deploiement TensorFlow Lite
...
Le modele par defaut de l'application est le MoE Expert, entraine avec le backbone EfficientNetB0 lors de ce projet. Il pese 17 Mo en TFLite.

Les trois modeles individuels (age, genre, ethnicite) pesent chacun 8.4 Mo. Le modele multitache pese 11 Mo. Le modele de detection de visage MediaPipe pese 0.2 Mo.

Tous les modeles ont ete re-entraines avec EfficientNetB0 comme backbone, selectionne grace a la comparaison du notebook 1.
...
L'application propose quatre modes de prediction selectionnables dans les parametres. Le mode MoE Expert, recommande par defaut, utilise le modele Mixture of Experts entraine lors de ce projet. Le mode Oriente V2 utilise les trois modeles specialises. Le mode Multitache utilise un seul modele unifie. Le mode Hybride combine les meilleurs modeles avec du Test Time Augmentation.
...
Le modele multitache MoE, qui est le modele par defaut de l'application, atteint 90.7 pourcent d'accuracy en genre, 70.8 pourcent en ethnicite et 6.36 ans de MAE en age.
...
La comparaison rapide des backbones confirme qu'EfficientNetB0 est le meilleur choix avec un score composite de 9.17 sur 10.
...
Nous avons implemente avec succes les trois strategies de modelisation demandees par le sujet et deploye le meilleur modele dans une application Android fonctionnelle et professionnelle.

Les performances obtenues sont satisfaisantes : 90.7 pourcent de precision sur le genre, 70.8 pourcent sur l'ethnicite, et 6.36 ans d'erreur moyenne sur l'age. L'application offre une experience utilisateur complete avec quatre modes de prediction, authentification Firebase, historique, mode temps reel, mode sombre, et un design moderne.

En perspectives d'amelioration, un dataset plus grand et mieux equilibre permettrait d'ameliorer les performances sur les classes sous-representees. Des architectures plus recentes comme EfficientNetV2, ConvNeXt ou les Vision Transformers pourraient etre testees. La quantification int8 reduirait la taille des modeles pour un deploiement plus leger. Enfin, la detection multi-visages permettrait de predire sur tous les visages detectes dans une meme image.